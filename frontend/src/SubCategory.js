...
const categories = [
    ...
    { name: '진료예약', subcategories: [{ name: '온라인예약', path: '2001' }, { name: 'AI추천', path: '2002' }, { name: '병원안내', path: '2003' }] },
            ...
            <div className="flex flex-col min-h-screen bg-sky-50">
                <header className="bg-white shadow-sm">
                    <div className="container mx-auto px-4 py-4">
                        ...
                        <nav className="mt-4 flex flex-wrap justify-center gap-24">
                            {categories.map((category, index) => (
                                <div
                                    key={index}
                                    className="relative"
                                    onMouseEnter={() => {
                                        console.log(`Hovering over: ${category.name}`); // Debug log
                                        setHoveredCategory(category.name);
                                    }}
                                    onMouseLeave={() => setHoveredCategory(null)}
                                >
                                    <span className="cursor-pointer text-sky-600 text-xl hover:font-bold hover:text-sky-800 transition-colors">
                                        {category.name}
                                    </span>
