...
export default function SignUpPage() {
    ...
    const validateForm = () => {
        ...
        if (!formData.password) {
            ...
            newErrors.password = '비밀번호는 최소 8자리 이상이며, 영문, 숫자, 특수문자를 포함해야 합니다.';
        ...
        if (!formData.rrn1 || !formData.rrn2) {
        ...
        } else if (!/^[0-9]{6}$/.test(formData.rrn1) || !/^[0-9]{7}$/.test(formData.rrn2)) {
            ...
            if (birthDate.getFullYear() !== yearPrefix + year ||
    ...
    const checkIdAvailability = async () => {
        try {
            const response = await axios.get(`http://localhost:8080/api/users/check-id/${formData.id}`);
            ...
            setErrorMessages(prev => ({...prev, id: '중복 확인 중 오류가 발생했습니다.'}));
        }
    };
                    ...
                    <div className="bg-green-100 rounded-full p-8 max-w-md mx-auto relative">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                            <div className="bg-sky-100 bg-opacity-75 px-4 py-2 rounded-lg">
                                <p className="text-xl font-bold text-blue-900 text-center">편리한 의료 서비스 <br/>메디곰에 오신 걸
                                    환영합니다.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
                        ...
                        <form className="space-y-6" onSubmit={handleSubmit}>
                            <div>
                                ...
                                {errorMessages.id && <p className="text-red-600 text-sm">{errorMessages.id}</p>}
                                {isIdAvailable !== null && (
                                    <p className={isIdAvailable ? "text-red-600 text-sm" : "text-green-600 text-sm"}>
                                        {isIdAvailable ? '사용 중인 아이디입니다.' : '사용 가능한 아이디입니다.'}
                                    </p>
                                )}
                            </div>
                            ...
                            <div className="flex space-x-2">
                                <div className="w-1/2">
                                    <label htmlFor="rrn1" className="block text-sm font-medium text-gray-700">
                                        주민등록번호 앞자리
                                    </label>
                                    <div className="mt-1">
                                        <input
                                ...
                                <div className="w-1/2">
                                    <label htmlFor="rrn2" className="block text-sm font-medium text-gray-700">
                                        주민등록번호 뒷자리
                                    </label>
                                    <div className="mt-1">
                                        <input
                            ...
                            {errorMessages.rrn && <p className="text-red-600 text-sm">{errorMessages.rrn}</p>}
                            ...
                            <div>
                                ...
                                <div className="mt-1">
                                    ...
                                    />
                                    {errorMessages.phone &&
                                        <p className="text-red-600 text-sm">{errorMessages.phone}</p>}
                                </div>
                            </div>
