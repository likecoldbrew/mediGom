...
import { Moon, Sun, Search, Filter, List, Grid, ChevronUp, ChevronDown, MoreVertical } from 'lucide-react';
...
const PatientList = () => {
    ...
    useEffect(() => {
        ...
        if (statusFilter !== 'All Status') {
            result = result.filter(patient => patient.deleteYn.toLowerCase() === statusFilter.toLowerCase());
        }
                            ...
                            <tr key={patient.userNo}>
                                <td className="px-6 py-4 whitespace-nowrap text-center">
                                    <div className="flex items-center justify-center">
                                        <div className="text-sm font-medium">{patient.userNo}</div>
                                    </div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-center">{patient.userId}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-center">
                                      <span
                                          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                              patient.deleteYn === 'N' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                          }`}
                                      >
                                        {patient.userName}
                                      </span>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-center">{patient.phone}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-center">{patient.email}</td>
                            </tr>
                        ))}
                        </tbody>
                    </table>
