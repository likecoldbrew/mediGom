...
function App() {
    ...
    const ClickChargeBtn = (pg_method, userNo, amount, redirect_url) => {
        ...
        IMP.request_pay({
                ...
                if (rsp.success) { // 만약 결제가 성공적으로 이루어졌다면
                    axios.get(`http://localhost:8080/verify/` + rsp.imp_uid + `?userNo=${userNo}`) // userNo 추가
                        .then((response) => {
    ...
    return (
        <div className="App">
            ...
            <button onClick={() => ClickChargeBtn('kakaopay', 123, amount,  'http://localhost:3000/redirect')}>카카오페이</button>
            <button onClick={() => ClickChargeBtn('tosspay', 123, amount,  'http://localhost:3000/redirect')}>토스페이</button>
        </div>
    );
}
